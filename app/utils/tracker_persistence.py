...
def save_tracker(tracker, filepath="tracker.pkl"):
...
def load_tracker(tracker, filepath="tracker.pkl"):
    """Load YOLOv8 BoT-SORT tracker state from disk"""
    try:
        with open(filepath, 'rb') as f:
            data = pickle.load(f)

            frame_id, count, track_len, prox_thresh, app_thresh, tracks, timestamp = data
        ...
        print("Not using local tracker save", e)
        return tracker