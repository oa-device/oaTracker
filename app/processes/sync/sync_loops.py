...
class SyncLoops:
    def __init__(self, args: Args, server_stopped: EventClass):
        ...
        self.thread_backup = Thread(
            target=self.backup_replication, args=(server_stopped,)
        )
        self.thread_quick = Thread(
            target=self.quick_replication, args=(server_stopped,)
        )
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    for root, _dirs, files in os.walk(warehouse_path, True):
                        for name in files:
