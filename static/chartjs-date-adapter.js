((global, factory) => {
	...
	function buildMatchFn(args) {
		return (dirtyString, dirtyOptions) => {
			...
			if (Object.prototype.toString.call(parsePatterns) === "[object Array]") {
				value = findIndex(parsePatterns, (pattern) =>
	...
	var formatters$2 = {
		...
		// Day of the month
		d: (date, token) => addLeadingZeros(date.getUTCDate(), token.length),
		// AM or PM
		a: (date, token) => {
		...
		// Hour [1-12]
		h: (date, token) =>
			addLeadingZeros(date.getUTCHours() % 12 || 12, token.length),
		// Hour [0-23]
		H: (date, token) => addLeadingZeros(date.getUTCHours(), token.length),
		// Minute
		m: (date, token) => addLeadingZeros(date.getUTCMinutes(), token.length),
		// Second
		s: (date, token) => addLeadingZeros(date.getUTCSeconds(), token.length),
		// Fraction of second
		S: (date, token) => {
	...
	var formatters = {
		...
		// Fraction of second
		S: (date, token) => formatters$3.S(date, token),
	...
	function parseNumericPattern(pattern, string, valueCallback) {
		...
		var value = Number.parseInt(matchResult[0], 10);
		return {
	...
	function parseTimezonePattern(pattern, string) {
		...
		var hours = matchResult[2] ? Number.parseInt(matchResult[2], 10) : 0;
		var minutes = matchResult[3] ? Number.parseInt(matchResult[3], 10) : 0;
		var seconds = matchResult[5] ? Number.parseInt(matchResult[5], 10) : 0;
		return {
			value:
				sign *
	...
	var parsers = {
		...
		G: {
			priority: 140,
			parse: (string, token, match, _options) => {
				switch (token) {
					// AD, BC
					case "G":
					case "GG":
		...
		y: {
			...
			validate: (_date, value, _options) =>
				value.isTwoDigitYear || value.year > 0,
			set: (date, flags, value, _options) => {
		...
		R: {
			priority: 130,
			parse: (string, token, _match, _options) => {
				if (token === "R") {
			...
			set: (_date, _flags, value, _options) => {
		...
		u: {
			...
			set: (date, _flags, value, _options) => {
				date.setUTCFullYear(value, 0, 1);
				date.setUTCHours(0, 0, 0, 0);
				return date;
			},
		...
		Q: {
			...
			validate: (_date, value, _options) => value >= 1 && value <= 4,
			set: (date, _flags, value, _options) => {
		...
		M: {
			...
			validate: (_date, value, _options) => value >= 0 && value <= 11,
			set: (date, _flags, value, _options) => {
		...
		w: {
			...
			validate: (_date, value, _options) => value >= 1 && value <= 53,
			set: (date, _flags, value, options) =>
				startOfUTCWeek(setUTCWeek(date, value, options), options),
			incompatibleTokens: [
				"y",
				"R",
				"u",
		...
		I: {
			...
			validate: (_date, value, _options) => value >= 1 && value <= 53,
			set: (date, _flags, value, options) =>
				startOfUTCISOWeek(setUTCISOWeek(date, value, options), options),
			incompatibleTokens: [
				"y",
				"Y",
				"u",
		...
		E: {
			...
			validate: (_date, value, _options) => value >= 0 && value <= 6,
			set: (date, _flags, value, options) => {
		...
		i: {
			...
			validate: (_date, value, _options) => value >= 1 && value <= 7,
			set: (date, _flags, value, options) => {
		...
		h: {
			...
			validate: (_date, value, _options) => value >= 1 && value <= 12,
			set: (date, _flags, value, _options) => {
		...
		H: {
			...
			validate: (_date, value, _options) => value >= 0 && value <= 23,
			set: (date, _flags, value, _options) => {
				date.setUTCHours(value, 0, 0, 0);
				return date;
			},
		...
		k: {
			...
			validate: (_date, value, _options) => value >= 1 && value <= 24,
			set: (date, _flags, value, _options) => {
		...
		m: {
			...
			validate: (_date, value, _options) => value >= 0 && value <= 59,
			set: (date, _flags, value, _options) => {
		...
		t: {
			priority: 40,
			parse: (string, _token, _match, _options) => parseAnyDigitsSigned(string),
			set: (_date, _flags, value, _options) => [
				new Date(value * 1000),
				{
					timestampIsSet: true,
	...
	function parseYear(dateString, additionalDigits) {
		...
		var year = captures[1] && Number.parseInt(captures[1]);
		var century = captures[2] && Number.parseInt(captures[2]);
		return {
	...
	function parseTimezone(timezoneString) {
		...
		var hours = Number.parseInt(captures[2]);
		var minutes = (captures[3] && Number.parseInt(captures[3])) || 0;
	...
	function fmtDuration(milliseconds, options = {}) {
	...
	chart_js._adapters._date.override({
		...
		format: (time, fmt) => fmtDuration(Date.now() - time),
